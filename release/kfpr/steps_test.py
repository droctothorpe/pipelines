...
class StepUpdateSdkVersionsTest(unittest.TestCase):
  ...
  def test_step_update_sdk_versions_updates_all_sdk_release_files(self):
    with TemporaryDirectory() as tmpdir:
      root = Path(tmpdir)
      files = {
          ...
          'kubernetes_platform/python/docs/conf.py': (
              ...
              "        {\n"
              "            'version':\n"
              "                'https://kfp-kubernetes.readthedocs.io/en/kfp-kubernetes-1.2.3/',\n"
              "            'title':\n"
              "                '1.2.3',\n"
              "            'aliases': ['stable'],\n"
              "        },\n"
      ...
      self.assertIn("'title':\n                '1.3.0'", (root / 'kubernetes_platform/python/docs/conf.py').read_text())
      ...
      self.assertIn(
          ['git', 'add', 'api/v2alpha1/python', 'backend/api/v2beta1/python_http_client', 'sdk', 'kubernetes_platform/python'],
          commands,
      )


class StepMergeSdkPrTest(unittest.TestCase):

  def test_step_merge_sdk_pr_uses_version_tag_update_branch(self):
    with TemporaryDirectory() as tmpdir:
      root = Path(tmpdir)
      runner = core.CommandRunner(dry_run=True)
      context = core.ReleaseContext(
      ...
      steps.step_merge_sdk_pr(context)

      self.assertIn(['git', 'checkout', '-b', '3.2.0-update-sdk-version-tags'], runner.commands)
      self.assertIn(
          ['git', 'push', '--set-upstream', 'git@github.com:testuser/pipelines.git', '3.2.0-update-sdk-version-tags'],
          runner.commands,
      )
